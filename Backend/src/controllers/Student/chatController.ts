...
// =========================
// GET CHATS - UPDATED WITH PROPER UNREAD COUNT
// =========================
  ...
  try {
    ...
    const enhanced = await Promise.all(
      chats.map(async (chat) => {
        ...
        // FIXED: Properly count unread messages (only from teacher, not read yet)
        const unreadCount = chat.messages?.filter(
          (m: any) => 
            m.sender.toString() !== studentId && 
            m.senderModel === 'Teacher' && // Ensure it's from teacher
            m.status === 'sent' // Only count unread messages
        ).length || 0;
...
// =========================
// GET MESSAGES - UPDATED WITH MESSAGE MARKING
// =========================
export const getMessages = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // FIXED: Mark ONLY teacher messages as read when student opens chat
    if (studentId) {
        ...
        { 
          arrayFilters: [
            { 
              ...
              "elem.senderModel": "Teacher", // Only mark teacher messages as read
              "elem.status": "sent" 
            }
          ] 
        }
...
// =========================
// SEND MESSAGE - UPDATED WITH BETTER REAL-TIME
// =========================
